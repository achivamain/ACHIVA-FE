...
import type { Notification } from "@/types/responses";
...
function NotificationItem({
  ...
  const Icon = cheeringMeta[notification.cheeringCategory as keyof typeof cheeringMeta].icon;
  const color = cheeringMeta[notification.cheeringCategory as keyof typeof cheeringMeta].color;

  const { data: post } = useQuery({
    queryKey: ["post", notification.articleId],
    queryFn: async () => {
      const res = await fetch(`/api/posts?postId=${notification.articleId}`);
      if (!res.ok) throw new Error("Failed to fetch post");
      const json = await res.json();
      return json.data as PostRes;
    },
    enabled: !!notification.articleId,
  ...
  return (
    <div>
      {showGroupHeader && (
        <Link
          href={`/post/${notification.articleId}`}
          className={`flex items-center gap-3 ${indexInList === 0 ? "" : "mt-5"
            } mb-3`}
        >
          <div className="font-semibold text-xl bg-theme text-white rounded-sm px-4 py-1.5 min-w-32 text-center">
            {post ? (post.bookArticle?.[0]?.bookTitle || post.category) : "..."}
          </div>
          <p className="text-[#808080] font-light">
            {post ? `${post.authorCategorySeq}번째 이야기` : ""}
          </p>
      ...
      <li
        className={`flex gap-2.5 items-center py-2 ${showGroupHeader ? "" : "border-t border-t-black/15"
          }`}
      >
        <Link href={`/${notification.senderName}`}>
          <ProfileImg url={notification.senderProfileImageUrl} size={50} />
        </Link>
        <div className="flex-1 flex gap-2.5 items-center">
          <Link href={`/${notification.senderName}`} className="font-semibold">
          ...
          >
            <p>{notification.cheeringCategory}</p>
            <Icon active />
          </div>
        </div>
      </li>

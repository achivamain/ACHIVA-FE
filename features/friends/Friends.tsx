...
export default function Friends({ nickName, isMe }: Props) {
  ...
  const isPending = friends.isPending || (isMe && receivedFriendRequests.isPending);
    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex gap-2 mb-5 sm:mb-8">
        <button
          ...
          className={`rounded-full font-semibold text-lg px-4 py-2 ${selectedMenu === "친구 목록"
            ? "bg-theme text-white"
        ...
        {isMe && (
          <button
            ...
            className={`relative rounded-full font-semibold text-lg px-4 py-2  ${selectedMenu === "친구 요청"
              ? "bg-theme text-white"
              : "bg-white text-theme border border-theme"
              }`}
          >
